...
class Ldap extends Native
{
    ...
    private function getMemberDetail(string $memberId)
    {
        ...
        //         m.jenjang m_jenjang,
        //         m.jurusan as m_jurusan,
        //         m.program_studi as m_prodi
        //     FROM 
        //         member AS m 
        //         LEFT JOIN 
        ...
        if ($_entries) {
            ...
            if (count($this) < 1) {
                ...
                $member_type = '3';
                    ...
                    $alamat='Test';
                    $reg=date('Y-m-d');
                    $email=$_entries[0][$member_ldap_configs['mail_field']][0];
                ...
                //$entryData['expire_date'] = '2026-02-06';
                //$entryData['register_date'] = $now;
                $entryData['is_pending'] = '1';
                //$entryData['member_type_id'] = '1';
                $entryData['input_date'] = $_curr_date;
                $entryData['last_update'] = $_curr_date;
                $entryData['member_id'] = str_replace("'","",$memberId);
                $entryData['member_name'] = str_replace("'","",$_entries[0][$_fullname_field][0]);
                //$this->user_info['gender'] = '0';
                //tambahan sid
                //echo 'inik jk'.$jk;
                $entryData['birth_date'] = $lahir;
                $entryData['gender'] = $jk;
                $entryData['jenjang'] = $jenjang;
                // $$entryData['jurusan'] = $jurusan;
                $entryData['program_studi'] = $prodi;
                $entryData['member_phone'] = $hp;
                $entryData['member_address'] = $alamat;                
                $entryData['inst_name'] = 'Perpustakaan Polibatam';
                $entryData['member_email_address'] = str_replace("'","",$_entries[0][$member_ldap_configs['mail_field']][0]);
                $entryData['member_email'] = str_replace("'","",$email);
                //tanggal per semester
                $entryData['expire_date'] = '2026-02-06';
                //$this->user_info['birth_date'] = '2023-01-01';
                $entryData['register_date'] = $reg;
                $entryData['member_type_id'] = $member_type;
                $entryData['member_since_date'] = $_curr_date;
                ...
                if (!$_insert) throw new Exception(__('Member insertion error because of:') . ' test est eteh goreng' . $_dbop->error, 500);
                $this->getMemberDetail($memberId);
                echo $memberId;
