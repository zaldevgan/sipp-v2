 ...
 /**
    ...
    protected function ldapLogin() {
        ...
        if ($_entries) {
            ...
            $_check_q = $this->obj_db->query("SELECT m.rfid, m.gender, m.birth_date, m.member_email_address, m.member_address, m.member_phone, m.member_id, m.member_name, m.inst_name,
            m.member_email, m.expire_date, m.register_date, m.is_pending, m.member_image,m.jenjang, m.jurusan as jur, m.program_studi,
            m.member_type_id, mt.member_type_name, mt.enable_reserve, mt.reserve_limit, c.cf_6d630 as jurusan, c.cf_dff81 as prodi
            FROM member AS m LEFT JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id LEFT JOIN member_custom as c on m.member_id=c.member_id
            ...
            if ($_check_q->num_rows < 1) {
                ...
                if (!$_insert) {
                    $this->errors = 'Member insertion error because of: '.$_dbop->error.'<br>testest'.print_r($_check_q->num_rows);
                }
    ...
    public function nativeLogin() {
        ...
        */
        $_sql_member_login = sprintf("SELECT m.rfid, m.gender, m.birth_date, m.member_email_address, m.member_address, m.member_phone, m.member_id, m.member_name, m.inst_name,
            m.member_email, m.expire_date, m.register_date, m.is_pending, m.member_image,m.jenjang, m.jurusan as jur, m.program_studi,
            m.member_type_id, mt.member_type_name, mt.enable_reserve, mt.reserve_limit, c.cf_6d630 as jurusan, c.cf_dff81 as prodi
            FROM member AS m LEFT JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id LEFT JOIN member_custom as c on m.member_id=c.member_id
